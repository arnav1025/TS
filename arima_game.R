...
{
  vec=rep(0,l)
  for (i in 1:l) vec[i]=runif(1,0.3,0.9)*sample(c(-1,1),1) # magnitude uniformly b/w 0.2 and 0.9 and random sign
  return(vec)
}
samp_theta=function(l)
{
  vec=rep(0,l)
  for (i in 1:l) vec[i]=runif(1,0.3,0.5)*sample(c(-1,1),1) # magnitude uniformly b/w 0.2 and 0.5 and random sign
  return(vec)
}
...
play_arima_game=function()
{
  ...
  n=sample(3e2:1e4,1) # length of time series
  ...
  phi=NULL; if (p>0) phi=samp_phi(p) # sample values b/w -1 and 1
  if (p==2) 
  {
    # resample values for phi b/w -1 and 1 until staionarity is ok
    #while (!stationary_AR2(phi))
    #  phi=samp_phi(p) # sample values b/w -1 and 1
    phistar=samp_phi(p)
    phi=c(sum(phistar),-prod(phistar))
  }
  theta=NULL; if (q>0) theta=samp_theta(q) # sample values b/w -1 and 1
  y=arima.sim(model=list(ar=phi,ma=theta,order=c(p,d,q)),n=n)

  # first allow the forecast package to make an automated guess
  auto.res=auto.arima(y,allowmean=FALSE,seasonal=F)
